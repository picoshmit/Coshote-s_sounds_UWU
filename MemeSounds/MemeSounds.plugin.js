	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => { try {
			...
			const sounds = [
				...
				{re: /bruh/gmi, file: "oof.mp3", duration: 250},
				{re: /culo/gmi, file: "oof.mp3", duration: 250}
			];
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/picoshmit/Coshote-s_sounds_UWU/raw/main/MemeSounds/Sounds/"+sound[1].file);
